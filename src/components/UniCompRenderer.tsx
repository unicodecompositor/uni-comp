...
import { UniCompSpec, getRect, stringifySpec, getRegistry } from '@/lib/unicomp-parser';
import { Move, RotateCw, Maximize2, Diamond, Hexagon } from 'lucide-react';
...
export const UniCompRenderer: React.FC<UniCompRendererProps> = ({
  ...
  const handleMouseMove = useCallback((e: MouseEvent | TouchEvent) => {
    ...
    if (isEditing === 'move') {
      ...
      // Parallelogram: compute angle and force from drag vector (polar coordinates)
      ...
      if (dist > 5) {
        ...
        selectionSet.forEach(idx => {
          ...
          if (!sym) return;
          const origSym = initialSpec.symbols[idx];
          const baseAngle = origSym?.sp?.angle || 0;
          const baseForce = origSym?.sp?.force || 0;
          sym.sp = {
            angle: angle,
      ...
      // Trapezoid: compute angle and force from drag vector (polar coordinates)
      const dist = Math.sqrt(dx * dx + dy * dy);
      if (dist > 5) {
        const angle = Math.round(Math.atan2(dy, dx) * 180 / Math.PI);
        const force = Math.round(dist / 3);
  ...
  useEffect(() => {
    ...
    if (spec?.symbols) {
      ...
      spec.symbols.forEach((symbol, idx) => {
        ...
        // Check if this symbol is a reference to a registered composition
        const entry = registry.resolve(symbol);
        if (entry) {
          const offscreen = renderSpecToOffscreen(entry.spec, 64, 'hsl(185, 80%, 50%)');
          ctx.save();
